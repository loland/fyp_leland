...
import com.example.mymap.Utility.BoundingBox;
...
import com.example.mymap.customview.OverlayView.DrawCallback;
import com.example.mymap.env.BorderedText;
import com.example.mymap.env.ImageUtils;
import com.example.mymap.env.Logger;
import com.example.mymap.tflite.Classifier;
...
public class DetectorActivity extends CameraActivity implements OnImageAvailableListener {
    ...
    public static final Size DESIRED_PREVIEW_SIZE = new Size(640, 640);
    ...
    public void onPreviewSizeChosen(Size size, int rotation) {
        ...
        try {
            Log.d("DetectorActivity/onPreviewSizeChosen", "modelString: " + modelString);
    ...
    protected void processImage() {
        ...
        Log.d("DetectorActivity/processImage", "croppedBitmap width: " + croppedBitmap.getWidth() + ", height: " + croppedBitmap.getHeight());
...
//                        }
                        ...
                        for (final Classifier.Recognition result : results) {
                            final BoundingRectF location = result.getLocation();

//                            Log.d("DetectorActivity/run", String.format("width: %s, height: %s, X: %s, Y: %s, imageWidth: %s, imageHeight: %s", width, height, x, y, imageWidth, imageHeight));
                            if (location != null && result.getConfidence() >= minimumConfidence) {
                                ...
                                boundingBoxes.add(new BoundingBox(location, result.getDetectedClass()));
                            }
                        }

                        if (sendRetrainingImages) {
                            if (retrainingIndex == Settings.retrainingCount) {
                                String json = BoundingBox.getJsonString(croppedBitmap, boundingBoxes, detector.detectorName, croppedBitmap.getWidth(), croppedBitmap.getHeight());
                                HttpRequest.makePostRequestWithoutResponse(json, Settings.retrainingServer + Settings.sendImage);
                                retrainingIndex = 0;
                            } else if (boundingBoxes.size() > 0) {
                                retrainingIndex += 1;
                            }
                        }

//                        Log.d("DetectorActivity/processImage", "boundingBoxes.length(): " + boundingBoxes.size());
